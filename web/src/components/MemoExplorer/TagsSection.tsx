    ...
    <div className="w-full flex flex-col justify-start items-start mt-3 px-1 h-auto shrink-0 flex-nowrap hide-scrollbar">
      ...
      {tags.length > 0 ? (
        treeMode ? (
          ...
          <div className="w-full flex flex-row justify-start items-center relative flex-wrap gap-x-2 gap-y-1.5 border border-sidebar-border/80 bg-card/70 px-3 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.4)]">
            {tags.map(([tag, amount]) => {
              const isActive = getFiltersByFactor("tagSearch").some((filter: MemoFilter) => filter.value === tag);
              return (
                <div

...
func (d *DB) CreateBank(ctx context.Context, create *store.Bank) (*store.Bank, error) {
	...
	stmt := "INSERT INTO `bank` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ") RETURNING `id`, `created_ts`, `updated_ts`"
...
func (d *DB) CreateLoan(ctx context.Context, create *store.Loan) (*store.Loan, error) {
	...
	stmt := "INSERT INTO `loan` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ") RETURNING `id`, `created_ts`, `updated_ts`"
	if err := d.db.QueryRowContext(ctx, stmt, args...).Scan(
		&create.ID,
		&create.CreatedTs,
		&create.UpdatedTs,
...
func (d *DB) UpdateLoan(ctx context.Context, update *store.UpdateLoan) error {
	...
	if v := update.StartTime; v != nil {
		set, args = append(set, "`start_time` = ?"), append(args, v.Unix())
	}
	if v := update.NextRepaymentTime; v != nil {
		set, args = append(set, "`next_repayment_time` = ?"), append(args, v.Unix())
	}
...
func (d *DB) CreateRepayment(ctx context.Context, create *store.Repayment) (*store.Repayment, error) {
	...
	stmt := "INSERT INTO `repayment` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ") RETURNING `id`, `created_ts`, `updated_ts`"
	if err := d.db.QueryRowContext(ctx, stmt, args...).Scan(
		&create.ID,
		&create.CreatedTs,
		&create.UpdatedTs,
...
func (d *DB) UpdateRepayment(ctx context.Context, update *store.UpdateRepayment) error {
	...
	if v := update.RepaymentTime; v != nil {
		set, args = append(set, "`repayment_time` = ?"), append(args, v.Unix())
	}
	if v := update.PrincipalCents; v != nil {
		set, args = append(set, "`principal_cents` = ?"), append(args, *v)
	}

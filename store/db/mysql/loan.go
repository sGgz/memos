...
func (d *DB) CreateBank(ctx context.Context, create *store.Bank) (*store.Bank, error) {
	...
	stmt := "INSERT INTO `bank` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ")"
	...
	id := int32(rawID)
	return d.GetBank(ctx, &store.FindBank{ID: &id})
}
...
func (d *DB) ListBanks(ctx context.Context, find *store.FindBank) ([]*store.Bank, error) {
	...
	if v := find.ID; v != nil {
		where, args = append(where, "`bank`.`id` = ?"), append(args, *v)
	}
	if v := find.UID; v != nil {
		where, args = append(where, "`bank`.`uid` = ?"), append(args, *v)
	}

	fields := []string{
		"`bank`.`id` AS `id`",
		"`bank`.`uid` AS `uid`",
		"UNIX_TIMESTAMP(`bank`.`created_ts`) AS `created_ts`",
		"UNIX_TIMESTAMP(`bank`.`updated_ts`) AS `updated_ts`",
		"`bank`.`title` AS `title`",
		"`bank`.`description` AS `description`",
	}

	query := "SELECT " + strings.Join(fields, ", ") + " FROM `bank` WHERE " + strings.Join(where, " AND ") + " ORDER BY `id` DESC"
	if find.Limit != nil {
...
func (d *DB) UpdateBank(ctx context.Context, update *store.UpdateBank) error {
	...
	stmt := "UPDATE `bank` SET " + strings.Join(set, ", ") + " WHERE `id` = ?"
...
func (d *DB) DeleteBank(ctx context.Context, delete *store.DeleteBank) error {
	stmt := "DELETE FROM `bank` WHERE `id` = ?"
...
func (d *DB) CreateLoan(ctx context.Context, create *store.Loan) (*store.Loan, error) {
	...
	stmt := "INSERT INTO `loan` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ")"
	result, err := d.db.ExecContext(ctx, stmt, args...)
	if err != nil {
		return nil, err
	}
	...
	id := int32(rawID)
	return d.GetLoan(ctx, &store.FindLoan{ID: &id})
}
...
func (d *DB) ListLoans(ctx context.Context, find *store.FindLoan) ([]*store.Loan, error) {
	...
	if v := find.ID; v != nil {
		where, args = append(where, "`loan`.`id` = ?"), append(args, *v)
	}
	if v := find.UID; v != nil {
		where, args = append(where, "`loan`.`uid` = ?"), append(args, *v)
	}
	if v := find.BankID; v != nil {
		where, args = append(where, "`loan`.`bank_id` = ?"), append(args, *v)
	}

	fields := []string{
		"`loan`.`id` AS `id`",
		"`loan`.`uid` AS `uid`",
		"UNIX_TIMESTAMP(`loan`.`created_ts`) AS `created_ts`",
		"UNIX_TIMESTAMP(`loan`.`updated_ts`) AS `updated_ts`",
		"`loan`.`title` AS `title`",
		...
		"UNIX_TIMESTAMP(`loan`.`start_time`) AS `start_time`",
		"UNIX_TIMESTAMP(`loan`.`next_repayment_time`) AS `next_repayment_time`",
	...
	query := "SELECT " + strings.Join(fields, ", ") + " FROM `loan` WHERE " + strings.Join(where, " AND ") + " ORDER BY `order_index` ASC, `id` DESC"
	if find.Limit != nil {
		query = fmt.Sprintf("%s LIMIT %d", query, *find.Limit)
		if find.Offset != nil {
			query = fmt.Sprintf("%s OFFSET %d", query, *find.Offset)
...
func (d *DB) UpdateLoan(ctx context.Context, update *store.UpdateLoan) error {
	...
	if v := update.BankID; v != nil {
		set, args = append(set, "`bank_id` = ?"), append(args, *v)
	}
	if v := update.PrincipalCents; v != nil {
		set, args = append(set, "`principal_cents` = ?"), append(args, *v)
	}
	if v := update.InterestRateBps; v != nil {
		set, args = append(set, "`interest_rate_bps` = ?"), append(args, *v)
	}
	if v := update.StartTime; v != nil {
		set, args = append(set, "`start_time` = FROM_UNIXTIME(?)"), append(args, v.Unix())
	}
	if v := update.NextRepaymentTime; v != nil {
		set, args = append(set, "`next_repayment_time` = FROM_UNIXTIME(?)"), append(args, v.Unix())
	}
	if v := update.RepaidPrincipalCents; v != nil {
		set, args = append(set, "`repaid_principal_cents` = ?"), append(args, *v)
	}
	if v := update.RepaidInterestCents; v != nil {
		set, args = append(set, "`repaid_interest_cents` = ?"), append(args, *v)
	}
	if v := update.RepaidAmountCents; v != nil {
		set, args = append(set, "`repaid_amount_cents` = ?"), append(args, *v)
	}
	if v := update.RemainingPrincipalCents; v != nil {
		set, args = append(set, "`remaining_principal_cents` = ?"), append(args, *v)
	}
	if v := update.OrderIndex; v != nil {
		set, args = append(set, "`order_index` = ?"), append(args, *v)
	}
	if v := update.MonthlyRepaymentDay; v != nil {
		set, args = append(set, "`monthly_repayment_day` = ?"), append(args, *v)
	}
	if v := update.RepaymentPeriods; v != nil {
		set, args = append(set, "`repayment_periods` = ?"), append(args, *v)
	}
	if v := update.RepaymentMethod; v != nil {
		set, args = append(set, "`repayment_method` = ?"), append(args, *v)
	}
	if v := update.UpdatedTs; v != nil {
		set, args = append(set, "`updated_ts` = FROM_UNIXTIME(?)"), append(args, *v)
	}
	...
	stmt := "UPDATE `loan` SET " + strings.Join(set, ", ") + " WHERE `id` = ?"
	if _, err := d.db.ExecContext(ctx, stmt, args...); err != nil {
		return err
	}
	return nil
...
func (d *DB) DeleteLoan(ctx context.Context, delete *store.DeleteLoan) error {
	stmt := "DELETE FROM `loan` WHERE `id` = ?"
	result, err := d.db.ExecContext(ctx, stmt, delete.ID)
	if err != nil {
		return err
	}
...
func (d *DB) CreateRepayment(ctx context.Context, create *store.Repayment) (*store.Repayment, error) {
	...
	placeholder := []string{"?", "?", "FROM_UNIXTIME(?)", "?", "?", "?", "?", "?", "?", "?"}
	isEarly := 0
	...
	stmt := "INSERT INTO `repayment` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ")"
	result, err := d.db.ExecContext(ctx, stmt, args...)
	if err != nil {
		return nil, err
	}
	...
	id := int32(rawID)
	return d.GetRepayment(ctx, &store.FindRepayment{ID: &id})
}
...
func (d *DB) ListRepayments(ctx context.Context, find *store.FindRepayment) ([]*store.Repayment, error) {
	...
	if v := find.ID; v != nil {
		where, args = append(where, "`repayment`.`id` = ?"), append(args, *v)
	}
	if v := find.UID; v != nil {
		where, args = append(where, "`repayment`.`uid` = ?"), append(args, *v)
	}
	if v := find.LoanID; v != nil {
		where, args = append(where, "`repayment`.`loan_id` = ?"), append(args, *v)
	}

	fields := []string{
		"`repayment`.`id` AS `id`",
		"`repayment`.`uid` AS `uid`",
		"UNIX_TIMESTAMP(`repayment`.`created_ts`) AS `created_ts`",
		"UNIX_TIMESTAMP(`repayment`.`updated_ts`) AS `updated_ts`",
		"`repayment`.`loan_id` AS `loan_id`",
		"UNIX_TIMESTAMP(`repayment`.`repayment_time`) AS `repayment_time`",
	...
	query := "SELECT " + strings.Join(fields, ", ") + " FROM `repayment` WHERE " + strings.Join(where, " AND ") + " ORDER BY `repayment_time` DESC, `id` DESC"
	if find.Limit != nil {
		query = fmt.Sprintf("%s LIMIT %d", query, *find.Limit)
		if find.Offset != nil {
			query = fmt.Sprintf("%s OFFSET %d", query, *find.Offset)
...
func (d *DB) UpdateRepayment(ctx context.Context, update *store.UpdateRepayment) error {
	...
	if v := update.RepaymentTime; v != nil {
		set, args = append(set, "`repayment_time` = FROM_UNIXTIME(?)"), append(args, v.Unix())
	}
	if v := update.PrincipalCents; v != nil {
		set, args = append(set, "`principal_cents` = ?"), append(args, *v)
	}
	if v := update.InterestCents; v != nil {
		set, args = append(set, "`interest_cents` = ?"), append(args, *v)
	}
	if v := update.TotalCents; v != nil {
		set, args = append(set, "`total_cents` = ?"), append(args, *v)
	}
	if v := update.RemainingPrincipalCents; v != nil {
		set, args = append(set, "`remaining_principal_cents` = ?"), append(args, *v)
	}
	if v := update.Note; v != nil {
		set, args = append(set, "`note` = ?"), append(args, *v)
	}
	if v := update.IsEarlyRepayment; v != nil {
		...
		set, args = append(set, "`is_early_repayment` = ?"), append(args, value)
	}
	if v := update.Period; v != nil {
		if *v == 0 {
			set = append(set, "`period` = NULL")
		} else {
			set, args = append(set, "`period` = ?"), append(args, *v)
		}
	}
	if v := update.UpdatedTs; v != nil {
		set, args = append(set, "`updated_ts` = FROM_UNIXTIME(?)"), append(args, *v)
	...
	stmt := "UPDATE `repayment` SET " + strings.Join(set, ", ") + " WHERE `id` = ?"
	if _, err := d.db.ExecContext(ctx, stmt, args...); err != nil {
		return err
	}
	return nil
...
func (d *DB) DeleteRepayment(ctx context.Context, delete *store.DeleteRepayment) error {
	stmt := "DELETE FROM `repayment` WHERE `id` = ?"
	result, err := d.db.ExecContext(ctx, stmt, delete.ID)
	if err != nil {
		return err
	}

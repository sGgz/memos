...
func (d *DB) CreateTodo(ctx context.Context, create *store.Todo) (*store.Todo, error) {
	...
	stmt := "INSERT INTO `todo` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ")"
	...
	id := int32(rawID)
	return d.GetTodo(ctx, &store.FindTodo{ID: &id})
}
...
func (d *DB) ListTodos(ctx context.Context, find *store.FindTodo) ([]*store.Todo, error) {
	...
	if v := find.ID; v != nil {
		where, args = append(where, "`todo`.`id` = ?"), append(args, *v)
	}
	if v := find.UID; v != nil {
		where, args = append(where, "`todo`.`uid` = ?"), append(args, *v)
	}
	if v := find.Status; v != nil {
		where, args = append(where, "`todo`.`status` = ?"), append(args, *v)
	}
	...
	fields := []string{
		...
		"UNIX_TIMESTAMP(`todo`.`created_ts`) AS `created_ts`",
		"UNIX_TIMESTAMP(`todo`.`updated_ts`) AS `updated_ts`",
		"`todo`.`title` AS `title`",
		"`todo`.`description` AS `description`",
		"UNIX_TIMESTAMP(`todo`.`due_time`) AS `due_time`",
		"`todo`.`priority` AS `priority`",
		"`todo`.`status` AS `status`",
		"`todo`.`payload` AS `payload`",
	}

	query := "SELECT " + strings.Join(fields, ", ") + " FROM `todo` WHERE " + strings.Join(where, " AND ") + " ORDER BY `due_time` ASC, `id` DESC"
	if find.Limit != nil {
...
func (d *DB) UpdateTodo(ctx context.Context, update *store.UpdateTodo) error {
	...
	if v := update.DueTime; v != nil {
		set, args = append(set, "`due_time` = FROM_UNIXTIME(?)"), append(args, v.Unix())
	}
	...
	stmt := "UPDATE `todo` SET " + strings.Join(set, ", ") + " WHERE `id` = ?"

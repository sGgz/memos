...
- 在服务层进行输入验证
- 通过参数化查询防止 SQL 注入
## 长时运行 Agent Harness 学习笔记（2025-11-26）

以下内容用于指导 AI 代理在跨多个上下文窗口的长任务中稳定推进，避免“干一半断片”或“提前宣布完工”。

### 核心结论

- 采用**双阶段代理**：
  - 初始化代理（仅首轮）负责脚手架与规则落地。
  - 编码代理（后续每轮）只做增量交付并留下可追踪记录。
- 单靠上下文压缩不够，必须引入**持久化工件**（feature list / progress log / git commit）。
- 每轮结束前必须把环境恢复到“可合并状态”：代码可运行、问题有记录、改动可追溯。

### 建议落地工件

- `init.sh`：一键启动开发环境并执行最小冒烟路径。
- `feature_list.json`：结构化功能清单，建议字段：`category`、`description`、`steps`、`passes`。
- `progress log`（如 `claude-progress.txt`）：记录本轮目标、执行、验证结果、遗留问题。
- Git 提交：每轮至少一个语义明确的提交，必要时可回滚到稳定点。

### 会话启动固定流程（每个新窗口都执行）

1. `pwd` 确认工作目录。
2. 读取进度日志与最近提交历史（至少 `git log --oneline -20`）。
3. 读取 `feature_list.json`，选择**一个**最高优先级且未通过的功能。
4. 执行 `init.sh` 或等价启动流程。
5. 先跑最小 E2E 冒烟，确认当前基线可用，再开始开发。

### 编码阶段约束

- 一次只推进一个功能点，拒绝“一口吃成胖子”。
- 不允许删改功能清单的测试描述，仅可更新 `passes` 状态。
- 只有在完成端到端验证后，才允许把 `passes` 置为 `true`。
- 若发现历史遗留故障，优先修复基线，再做新功能。
...
- 补齐进度日志：
  ...
  - 怎么验证的；
  - 还有哪些风险和下一步建议。
- 保证工作区干净、服务状态明确、代码可被下一轮直接接手。
- 产出清晰 commit message，避免“misc/fix stuff”这类废话提交。

### 常见失败模式与应对

- **提前宣布完成**：靠 feature list 强制“未通过功能可见化”。
- **会话断档后无法接力**：靠 progress log + git history 还原上下文。
- **误判功能完成**：强制浏览器自动化/E2E 验证，不只看单测或接口返回。
- **新会话先写功能导致雪崩**：先做基线冒烟，坏了先修。

### 在 Memos 仓库中的执行建议

- 前端验证优先使用移动端视口，并统一使用 `root/123` 账号流程（若环境允许）。
- 对 UI 变更优先附带可复现的自动化验证与截图工件。
- 每轮改动后至少执行受影响子系统的最小检查（例如 `go test` 指定包或 `web` 侧 lint）。

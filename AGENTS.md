...
- 前端测试统一在**手机 Web 视图**下进行（移动端 viewport），不要只测桌面端。
...
- 默认验证顺序：先运行 `pnpm lint`；如果改动会影响最终发布页面，再运行 `pnpm release`，然后回到仓库根目录运行 `scripts/build.sh`。
...
- 浏览器验证统一使用手机视口 `390x844`，优先检查真实渲染结果和计算样式，不只看 DOM 类名。

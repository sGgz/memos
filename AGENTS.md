...
- 如果需要浏览器实测发布版页面，发布版后端启动优先参考 `scripts/all_build.sh`：从仓库根目录执行 `bash scripts/all_build.sh -s`；其等价手动命令是在 `build/` 目录执行 `./memos --addr 192.168.1.38 --port 12345 --data ..\\run`（Windows 下对应 `memos.exe`）。
...
- 发布版后端测试启动时，默认使用 `192.168.1.38` 作为监听 IP；优先从仓库根目录执行 `bash scripts/all_build.sh -s`，其等价手动命令是在 `build/` 目录执行 `./memos --addr 192.168.1.38 --port 12345 --data ..\\run`（Windows 下对应 `memos.exe`）。
